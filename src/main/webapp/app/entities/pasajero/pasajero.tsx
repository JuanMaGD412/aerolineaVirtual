...
import { getEntities } from './pasajero.reducer';
...
export const Pasajero = () => {
  ...
  const pasajeroList = useAppSelector(state => state.pasajero.entities);
  const loading = useAppSelector(state => state.pasajero.loading);
  const totalItems = useAppSelector(state => state.pasajero.totalItems);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        {pasajeroList && pasajeroList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('documentoIdentidad')}>
                  <Translate contentKey="aerolineaVirtualApp.pasajero.documentoIdentidad">Documento Identidad</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('documentoIdentidad')} />
                </th>
                <th className="hand" onClick={sort('nacionalidad')}>
                  <Translate contentKey="aerolineaVirtualApp.pasajero.nacionalidad">Nacionalidad</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('nacionalidad')} />
                </th>
            ...
            <tbody>
              {pasajeroList.map((pasajero, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/pasajero/${pasajero.id}`} color="link" size="sm">
                      {pasajero.id}
                    </Button>
                  </td>
                  ...
                  <td>{pasajero.documentoIdentidad}</td>
                  <td>{pasajero.nacionalidad}</td>
                  <td>
                    {pasajero.fechaNacimiento ? (
                      <TextFormat type="date" value={pasajero.fechaNacimiento} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/pasajero/${pasajero.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/pasajero/${pasajero.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/pasajero/${pasajero.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={pasajeroList && pasajeroList.length > 0 ? '' : 'd-none'}>
