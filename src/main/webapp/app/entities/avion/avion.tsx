...
export const Avion = () => {
  ...
  const totalItems = useAppSelector(state => state.avion.totalItems);
  ...
  return (
    <div>
            ...
            <tbody>
              {avionList.map((avion, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/avion/${avion.id}`} color="link" size="sm">
                      {avion.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/avion/${avion.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/avion/${avion.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/avion/${avion.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={avionList && avionList.length > 0 ? '' : 'd-none'}>
