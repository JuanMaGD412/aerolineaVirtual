  ...
  return (
    <div>
      <h2 id="asiento-heading" data-cy="AsientoHeading">
        <Translate contentKey="aerolineaVirtualApp.asiento.home.title">Asientos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="aerolineaVirtualApp.asiento.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/asiento/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="aerolineaVirtualApp.asiento.home.createLabel">Create new Asiento</Translate>
          </Link>
        </div>
      </h2>
        ...
        {asientoList && asientoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="aerolineaVirtualApp.asiento.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('clase')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('clase')} />
                </th>
                <th className="hand" onClick={sort('disponible')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('disponible')} />
                </th>
                <th>
                  <Translate contentKey="aerolineaVirtualApp.asiento.vuelo">Vuelo</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {asientoList.map((asiento, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`aerolineaVirtualApp.ClaseAsiento.${asiento.clase}`} />
                  </td>
                  <td>{asiento.disponible ? 'true' : 'false'}</td>
                  <td>{asiento.vuelo ? <Link to={`/vuelo/${asiento.vuelo.id}`}>{asiento.vuelo.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="aerolineaVirtualApp.asiento.home.notFound">No Asientos found</Translate>
            </div>
          )
        )}
      </div>
